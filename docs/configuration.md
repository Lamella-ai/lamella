...
| `LAMELLA_APP_TZ` | `UTC` | IANA timezone name (e.g. `America/Denver`, `Europe/Berlin`). Affects rendering of all user-facing timestamps — transactions, receipts, audit log — via the `\|local_ts` Jinja filter. Invalid values fall back to UTC with a log warning. See [ADR-0023](adr/0023-datetimes-tz-aware-utc.md) for the timestamp rendering contract. **(runtime)** |
