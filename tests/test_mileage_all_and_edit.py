...
def test_mileage_edit_rejects_negative_miles(app_client, settings):
    ...
    # ADR-0066: validation errors re-render the form (422) with the
    # submitted values intact — no redirect.
...
def test_mileage_edit_delete_form_not_nested(app_client, settings):
    """Upstream lamella#9: the delete <form> was nested inside the edit
    <form>. Browsers drop the inner <form> tag, so the Delete button
    submitted the edit endpoint instead — delete behaved like a save
    (and the data-confirm modal never fired). The delete form must be
    a sibling of the edit form, with the button associated via the
    form= attribute."""
    db = sqlite3.connect(str(settings.db_path))
    _seed_vehicle(db)
    trip_id = _seed_trip(db, entry_date="2026-03-15", miles=42.5)
    db.commit()
    db.close()

    body = app_client.get(f"/mileage/{trip_id}/edit").text

    # No <form> opens between the edit form's open tag and its close.
    edit_open = body.index(f'action="/mileage/{trip_id}"')
    edit_close = body.index("</form>", edit_open)
    assert "<form" not in body[edit_open:edit_close]

    # The standalone delete form exists, targets the delete endpoint,
    # carries the confirm modal attributes, and the actions-row button
    # is associated with it.
    delete_open = body.index('id="delete-trip-form"')
    assert delete_open > edit_close
    delete_close = body.index("</form>", delete_open)
    delete_form = body[delete_open:delete_close]
    assert f'action="/mileage/{trip_id}/delete"' in delete_form
    assert "data-confirm=" in delete_form
    assert 'form="delete-trip-form"' in body

    # The confirm message names the trip date (driven_on never existed
    # on the row dict; it rendered as an empty string).
    assert "Delete the 2026-03-15 trip" in delete_form


def test_vehicle_detail_recent_trips_link_to_edit(app_client, settings):
    """Upstream lamella#9 follow-up: the vehicle page's Recent-trips
    table had no path to the trip editor."""
    db = sqlite3.connect(str(settings.db_path))
    _seed_vehicle(db)
    trip_id = _seed_trip(db, entry_date="2026-03-15")
    db.commit()
    ...
    assert resp.status_code == 200
    assert f"/mileage/{trip_id}/edit" in resp.text
