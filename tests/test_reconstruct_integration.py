...
def test_all_steps_round_trip_end_to_end(ledger_dir: Path, tmp_path):
    ...
    assert db.execute(
        "SELECT COUNT(*) AS n FROM document_dismissals"
    ).fetchone()["n"] == 1
    assert db.execute(
...
def test_reconstruct_refuses_non_empty_db_without_force(ledger_dir: Path, tmp_path):
    ...
    db.execute(
        "INSERT INTO document_dismissals (txn_hash, reason) VALUES (?, ?)",
...
def test_reconstruct_force_wipes_state_only(ledger_dir: Path, tmp_path):
    ...
    tables (paperless_doc_index, document_links, etc.) are preserved.
    """
    main_bean = ledger_dir / "main.bean"
    _prepare_opens(ledger_dir, "Expenses:Personal:Food:Groceries")
    append_rule(
    ...
    db.execute(
        "INSERT INTO document_dismissals (txn_hash) VALUES (?)",
        ("will-be-wiped",),
    )
    # Cache row that should survive.
    db.execute(
        "INSERT INTO document_links (paperless_id, txn_hash, txn_date, txn_amount) "
        "VALUES (?, ?, ?, ?)",
    ...
    # document_dismissals was wiped (no directive for "will-be-wiped").
    assert not db.execute(
        "SELECT 1 FROM document_dismissals WHERE txn_hash = ?",
        ("will-be-wiped",),
    ).fetchone()
    # document_links survived (cache table, not wiped).
    assert db.execute(
        "SELECT 1 FROM document_links WHERE txn_hash = ?",
        ("cached-link",),
    ).fetchone()
