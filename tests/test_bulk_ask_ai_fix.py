...
def test_empty_selection_htmx_returns_inline_error(app_client, monkeypatch):
    ...
    """
    # The configured-gate runs before the selection check; pin AI on so
    # this test exercises the empty-selection branch regardless of
    # whether the host env has an API key.
    monkeypatch.setattr(
        "lamella.core.config.Settings.ai_enabled",
...
def test_successful_submit_returns_job_modal(app_client, monkeypatch):
