...
def test_import_preview_flags_existing_duplicates(app_client, registered_vehicle):
    app_client.post(
        "/mileage/import/commit",
        data={
            "vehicle": registered_vehicle,
            "entity": "Personal",
            "text": "2025-01-01,69001,69011,seed\n",
        },
    )
    ...
    assert "duplicate — already recorded" in preview.text


def test_import_preview_flags_overlapping_range(app_client, registered_vehicle):
    app_client.post(
        "/mileage/import/commit",
        data={
            "vehicle": registered_vehicle,
            "entity": "Personal",
            "text": "2025-01-01,1000,1100,seed\n",
        },
    )
    ...
    preview = app_client.post(
        ...
        data={
            "vehicle": registered_vehicle,
            "entity": "Personal",
            "text": "2025-01-01,1050,1075,nested\n",
        },
    )
    assert preview.status_code == 200
    assert "overlapping odometer range" in preview.text


def test_import_commit_skips_overlapping_range(app_client, registered_vehicle):
    ...
    assert second.status_code == 200
    assert "Imported 0 trip" in second.text
    assert "overlapping odometer range" in second.text


def test_import_commit_allows_zero_mile_marker_inside_existing_span(
    app_client, registered_vehicle,
):
    first = app_client.post(
        "/mileage/import/commit",
        data={
            "vehicle": registered_vehicle,
            "entity": "Personal",
            "text": "2025-01-01,1000,1200,long drive\n",
        },
    )
    assert first.status_code == 200
    ...
    second = app_client.post(
        ...
        data={
            "vehicle": registered_vehicle,
            "entity": "Personal",
            "text": "2025-01-01,1100,1100,oil change no driving\n",
        },
    )
    assert second.status_code == 200
    assert "Imported 1 trip" in second.text
    assert "overlapping odometer range" not in second.text
