...
def test_ledger_registry_has_only_known_keys():
    ...
    bcg-* on-disk rewrite step, (("2", 3),) for the v2→v3 lineage
    stamping step, and (("3", 4),) for the v3→v4 step — and nothing
    else."""
    from lamella.core.bootstrap.detection import LATEST_LEDGER_VERSION
    expected = {
...
def test_all_migrations_dedupes_aliases():
    ...
    # Five distinct migrations today (sqlite catch-up, ledger v0→v1,
    # ledger v1→v2, ledger v2→v3, ledger v3→v4). Adding a new one
    # bumps this number; updating the assertion is the explicit
    # signal that the registry's surface area changed.
...
def test_find_for_finding_returns_none_for_unregistered_ledger_jump():
    ...
    # (4, 5) is unregistered — would correspond to a future v4→v5 step.
    f = _schema_drift_finding(axis="ledger", from_v=4, to_v=5)
    assert find_for_finding(f) is None


def test_find_for_finding_dispatches_ledger_v3_to_v4():
    """A schema_drift finding with axis='ledger', from='3', to=4 must
    resolve to MigrateLedgerV3ToV4."""
    f = _schema_drift_finding(axis="ledger", from_v="3", to_v=4)
    m = find_for_finding(f)
    assert m is _LEDGER_REGISTRY[("3", 4)]
    assert isinstance(m, MigrateLedgerV3ToV4)
