...
def test_append_dismissal_writes_valid_directive(ledger_dir: Path):
    ...
    # ADR-0061: writers emit document-dismissed going forward.
    assert 'custom "document-dismissed" "deadbeef"' in block
...
def test_reconstruct_rebuilds_dismissals(ledger_dir: Path, tmp_path):
    ...
    rows = db.execute(
        "SELECT txn_hash, reason FROM document_dismissals ORDER BY txn_hash"
    ).fetchall()
    assert len(rows) == 1
