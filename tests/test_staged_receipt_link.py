...
on every staged row, so the existing ``DocumentLinker`` works
...
def test_receipt_section_renders_for_staged_row(app_client):
    ...
    resp = app_client.get(
        f"/txn/{row.lamella_txn_id}/document-section",
        headers={"HX-Request": "true"},
    )
    assert resp.status_code == 200, resp.text
    body = resp.text
    # Section wrapper present so unlink swap-target works.
    assert 'id="staged-document-section"' in body
    # No links yet → empty-state copy.
    assert "No document linked yet." in body
    # Search box wired with the right hx-get URL.
    assert f'hx-get="/txn/{row.lamella_txn_id}/document-search"' in body


def test_receipt_section_rejects_legacy_hex_token(app_client):
    # Legacy hex (non-UUIDv7) tokens were retired in v3.
    resp = app_client.get(
        "/txn/abcdef1234567890abcdef1234567890abcdef12/document-section",
        headers={"HX-Request": "true"},
    )
...
def test_receipt_search_renders_paperless_candidates(app_client):
    ...
    with respx.mock(base_url="https://paperless.test") as mock:
        ...
        resp = app_client.get(
            f"/txn/{row.lamella_txn_id}/document-search",
            params={"q": "acme"},
            headers={"HX-Request": "true"},
        )
    assert resp.status_code == 200, resp.text
    ...
    assert f'hx-post="/txn/{row.lamella_txn_id}/document-link"' in body
...
def test_receipt_link_writes_directive_and_db_row(app_client, tmp_path):
    ...
    resp = app_client.post(
        f"/txn/{row.lamella_txn_id}/document-link",
    ...
    # Directive landed in connector_links.bean. ADR-0061: writers
    # emit document-link going forward.
    text = settings.connector_links_path.read_text(encoding="utf-8")
    assert f'custom "document-link" "{row.lamella_txn_id}"' in text
    assert "lamella-paperless-id: 101" in text

    # SQLite cache row mirrors the directive.
    cached = db.execute(
        "SELECT paperless_id, txn_hash, match_method "
        "  FROM document_links WHERE paperless_id = ?",
        (101,),
    ).fetchone()
...
def test_receipt_section_lists_linked_docs_after_link(app_client):
    ...
    assert resp.status_code == 200, resp.text
    body = resp.text
    assert "Linked documents" in body
    assert "#101" in body
    assert "Acme Receipt April" in body
    # And the per-row Unlink form points at the right endpoint.
    assert f'action="/txn/{row.lamella_txn_id}/document-unlink"' in body
...
def test_receipt_unlink_removes_directive_and_db_row(app_client):
    ...
    assert f'custom "document-link" "{row.lamella_txn_id}"' in pre_text

    resp = app_client.post(
        f"/txn/{row.lamella_txn_id}/document-unlink",
        data={"paperless_doc_id": "202"},
        headers={"HX-Request": "true"},
    )
    assert resp.status_code == 204, resp.text
    ...
    assert f'custom "document-link" "{row.lamella_txn_id}"' not in post_text
    # The unlink writes a document-link-blocked tombstone (reason
    # user_unlink) so auto-match won't silently relink the pair — the
    # doc id legitimately remains in the file on that directive.
    assert f'custom "document-link-blocked" "{row.lamella_txn_id}"' in post_text
    assert 'lamella-reason: "user_unlink"' in post_text

    # Cache row gone.
    cached = db.execute(
        "SELECT 1 FROM document_links WHERE paperless_id = 202 "
        "AND txn_hash = ?",
        (row.lamella_txn_id,),
    ).fetchone()
...
def test_receipt_unlink_keeps_other_links_intact(app_client):
    """Removing one document-link directive must NOT touch the others —
    ...
    # 302's document-link directive is gone; its id remains only on
    # the document-link-blocked tombstone the unlink writes.
    link_count = text.count(f'custom "document-link" "{row.lamella_txn_id}"')
    assert link_count == 2
    assert f'custom "document-link-blocked" "{row.lamella_txn_id}"' in text
...
def test_receipt_section_for_legacy_token_404s(app_client):
    ...
    for path in (
        f"/txn/{legacy}/document-section",
        f"/txn/{legacy}/document-search?q=x",
    ):
        resp = app_client.get(path, headers={"HX-Request": "true"})
        assert resp.status_code == 404, path

    for path in (
        f"/txn/{legacy}/document-link",
        f"/txn/{legacy}/document-unlink",
    ):
        resp = app_client.post(
            path,
