...
def _seed_paperless_doc(
    ...
    conn.execute(
        ...
        INSERT INTO paperless_doc_index
            (paperless_id, vendor, total_amount, document_date,
...
class TestLinked:
    def test_linked_receipt_returns_context(self, conn):
        ...
        ctx = fetch_document_context(conn, txn_hash="abc-txn")
    ...
    def test_unlinked_txn_hash_returns_none_linked(self, conn):
        _seed_paperless_doc(conn, paperless_id=1)
        ctx = fetch_document_context(conn, txn_hash="unknown-hash")
        assert ctx is None
...
class TestDegradation:
    ...
    def test_no_inputs_returns_none(self, conn):
        ...
        ctx = fetch_document_context(conn)
        assert ctx is None
