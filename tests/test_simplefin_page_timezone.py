...
def test_simplefin_ingest_started_uses_app_timezone(app_client):
    # Stored ingest timestamps are UTC; /simplefin should render in APP_TZ.
    # Insert into the APP's database (app.state.db) — the standalone
    # `db` fixture is a different SQLite file the routes never read.
...
def test_simplefin_ingest_started_date_only_stays_same_day(app_client):
    db = app_client.app.state.db
    db.execute(
        """
        INSERT INTO simplefin_ingests (
