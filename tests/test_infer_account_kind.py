...
class TestLiabilities:
    ...
    @pytest.mark.parametrize("path", [
        # No card keyword in any segment — the leaf-token pass can't
        # classify it; sibling-consensus inference (Phase 2 of
        # discovery) handles these at boot instead.
        "Liabilities:AcmeCo:BankOne:AffiliateD",
        "Liabilities:BetaCorp:BankOne:AffiliateD",
        "Liabilities:Personal:Chase:CardB",
    ])
    @pytest.mark.xfail(reason="keyword pass alone can't classify keyword-less leaves; covered by infer_kinds_by_sibling", strict=True)
    def test_credit_cards_keywordless_leaf(self, path: str):
        assert _infer_account_kind(path) == "credit_card"
