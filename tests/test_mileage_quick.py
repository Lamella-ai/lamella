...
def test_quick_post_allows_same_odometer_for_zero_mile_note(
    app_client, settings, ledger_dir,
):
    _open_entity(ledger_dir)
    db = sqlite3.connect(str(settings.db_path))
    ...
    resp = app_client.post(
        ...
        data={
            ...
            "vehicle_slug": "suvone",
            "odometer_end": "84000",
            "what": "Parked vehicle for service note",
            "category": "business",
        },
        follow_redirects=False,
    )
    ...
    db.row_factory = sqlite3.Row
    row = db.execute(
        "SELECT miles, odometer_start, odometer_end FROM mileage_entries "
        "WHERE entry_date = '2026-02-11'"
    ).fetchone()
    assert row is not None
    assert row["odometer_start"] == 84000
    assert row["odometer_end"] == 84000
...
def test_quick_post_rejects_unknown_category(app_client, settings):
    ...
    # error message and the submitted values — never a redirect that
    # clears the input.
    assert resp.status_code == 422
    assert "Invalid purpose" in resp.text
    assert 'value="84075"' in resp.text
...
def test_quick_post_rejects_unknown_vehicle(app_client, settings):
    ...
    # ADR-0066: re-render with error + preserved input, no redirect.
    assert resp.status_code == 422
    assert "Unknown vehicle" in resp.text
    assert 'value="nonexistent"' in resp.text
