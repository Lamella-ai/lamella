...
class InvalidWritebackFieldError(ValueError):
    """Raised when a writeback field name doesn't carry the
    ``Lamella_`` prefix per ADR-0044. The matcher MUST only write
    custom fields it owns; ad-hoc names ('vendor', 'receipt_date',
    'payment_last_four') collide with user-owned fields and break
    the namespace defense ADR-0003 / ADR-0044 establish.
...
# ADR-0044: the four canonical writeback custom fields the matcher
# creates and writes after a successful txn match. Order is stable
# so log lines (and tests) can iterate deterministically.
...
class PaperlessClient:
    ...
    async def writeback_lamella_fields(
        ...
        *,
        ...
        """
        # Fail loud + early on any non-Lamella_ name. The ADR is
        # explicit: writeback writes ONLY namespaced fields.
        for name in values.keys():
            if not name.startswith(LAMELLA_FIELD_PREFIX):
                raise InvalidWritebackFieldError(
                    f"writeback field name {name!r} must start with "
                    f"{LAMELLA_FIELD_PREFIX!r} per ADR-0044; "
                    f"non-Lamella_ fields are user-owned and the "
                    f"matcher MUST NOT write to them."
                )
        if not values:
            return {}
