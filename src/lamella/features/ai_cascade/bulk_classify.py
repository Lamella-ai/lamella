...
async def _classify_one(
    *,
    ...
    # Derive ``fixme_root`` from the FIXME-leg account so the prompt's
    # per-root preamble switches correctly (Income FIXME gets the
    # Income block, Liabilities FIXME gets the liability-payment
    # block, etc.). Without this kwarg the template defaults to the
    # Expenses ELSE-branch on every bulk-classify call regardless of
    # what root the staging path picked, so a deposit row whose
    # FIXME-leg is Income:Acme:FIXME ends up reading the EXPENSES
    # FIXME framing — guaranteed-wrong direction for the AI.
