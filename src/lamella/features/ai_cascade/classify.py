...
async def propose_account(
    ...
    *,
    ...
    receipt: ReceiptContext | None = None,
...
def build_classify_context(
    *,
    ...
    # bank-side leg per beancount's balanced-txn rule). A NEGATIVE
    # FIXME amount means the bank side was POSITIVE — a deposit /
    # credit / refund — which belongs in Income's box. Override the
    # whitelist root here so the AI sees Income:* candidates instead
    # of being constrained to expense leaves it can never propose
    # correctly. Keep Expenses for non-deposits and for the rare case
    # where the placeholder was already Income (sign-aware ingest).
    if (
        fixme_root == "Expenses"
