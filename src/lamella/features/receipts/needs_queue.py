...
isn't already linked to a receipt (via receipt_links), and isn't marked in
receipt_dismissals as "no receipt expected".
...
class NeedsReceiptItem:
    ...
    # (matches the receipt_links / receipt_dismissals schema).
...
def _linked_hashes(conn: sqlite3.Connection) -> set[str]:
    rows = conn.execute("SELECT DISTINCT txn_hash FROM receipt_links").fetchall()
    return {r["txn_hash"] for r in rows}


def _dismissed_hashes(conn: sqlite3.Connection) -> set[str]:
    rows = conn.execute("SELECT txn_hash FROM receipt_dismissals").fetchall()
    return {r["txn_hash"] for r in rows}
...
def find_orphan_dismissals(
    ...
    Returned rows mirror the ``receipt_dismissals`` schema plus the
...
def build_queue(
    *,
...
) -> tuple[list[tuple[NeedsReceiptItem, bool]], QueueStats]:
    ...
    out: list[tuple[NeedsReceiptItem, bool]] = []
    for entry in entries:
    ...
    if query or account_filter or min_amount is not None or max_amount is not None:
        ...
        filtered: list[tuple[NeedsReceiptItem, bool]] = []
