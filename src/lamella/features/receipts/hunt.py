    ...
    *,
    ...
    existing_links = {
        ...
        for row in conn.execute(
            f"SELECT txn_hash FROM document_links WHERE txn_hash IN "
            f"({','.join('?' * len(selected))})",
            tuple(selected),
        ).fetchall()
    }
...
def _process_one(
    *,
    ...
    # ADR-0061 Phase 5: column is ``document_date`` end-to-end — dict
    # keys, template reads, and downstream sorts all use ``document_date``
    # post Phase 5 rename.
    raw_pass1 = conn.execute(
        """
        SELECT pdi.paperless_id, pdi.vendor, pdi.document_date,
               pdi.total_amount, pdi.title, pdi.content_excerpt,
               pdi.created_date, pdi.document_type_id, pdi.document_type_name,
               CASE
                 WHEN pdi.document_date IS NOT NULL
                      AND ABS(julianday(pdi.document_date) - julianday(?)) <= ?
                 THEN 3
    ...
    # ORDER BY amount_score DESC, date_score DESC, document_date
    pass1_filtered.sort(
        key=lambda t: (-t[0], -t[1], t[2]["document_date"] or ""),
    )
    ...
    if not candidates and live_paperless is not None:
        try:
            ...
            if fetched_live:
                ...
                # ADR-0061 Phase 5: column is ``document_date`` end-to-end.
                raw_pass2 = conn.execute(
                    """
                    SELECT pdi.paperless_id, pdi.vendor,
                           pdi.document_date, pdi.total_amount,
            ...
            try:
                ...
                if ranked:
                    ...
                    raw_pass3 = conn.execute(
                        """
                        SELECT pdi.paperless_id, pdi.vendor,
                               pdi.document_date, pdi.total_amount, pdi.title,
                         ...
                         WHERE rl.paperless_id IS NULL
                           AND pdi.total_amount IS NOT NULL
                           AND pdi.document_date IS NOT NULL
                           AND ABS(julianday(pdi.document_date) - julianday(?)) <= ?
                         ORDER BY pdi.document_date
                        """,
