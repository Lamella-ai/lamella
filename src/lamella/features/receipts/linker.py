...
def remove_receipt_link(
    *,
    ...
    """Strip a single ``custom "receipt-link" "<txn_id>"`` block from
    ``connector_links.bean`` whose ``lamella-paperless-id`` matches
    ``paperless_id``. Returns ``True`` when a block was removed,
    ``False`` when nothing matched (idempotent on a no-op).
    ...
    Mirrors :class:`ReceiptLinker`'s contract: Beancount stays
    authoritative; the SQLite ``receipt_links`` row is also removed
    when ``conn`` is provided so callers don't end up with a dangling
    cache row pointing at a directive that no longer exists.
    The block shape produced by :class:`ReceiptLinker.link` is::
        YYYY-MM-DD custom "receipt-link" "<txn_id>"
    ...
    i = 0
    head_re = (
        r'^\d{4}-\d{2}-\d{2}\s+custom\s+"receipt-link"\s+"([^"]+)"\s*$'
    )
    import re as _re
...
class ReceiptLinker:
    """Write receipt_links rows AND stamp a meta line in connector_links.bean.
    ...
    def link(
        ...
        *,
        ...
        stamp = (
            f'\n{txn_date.isoformat()} custom "receipt-link" "{txn_hash}"\n'
        ...
        self.conn.execute(
            "DELETE FROM receipt_link_blocks "
            "WHERE paperless_id = ? AND txn_hash = ?",
            (paperless_id, txn_hash),
        )
        ...
        cursor = self.conn.execute(
            ...
            ON CONFLICT (paperless_id, txn_hash) DO UPDATE SET
                paperless_hash = COALESCE(excluded.paperless_hash, receipt_links.paperless_hash)
            """,
            (
                paperless_id,
                paperless_hash,
                txn_hash,
                txn_date.isoformat(),
                # ADR-0022: receipt_links.txn_amount is TEXT (migration 057);
        ...
        if self.run_check:
            try:
                ...
                self.conn.execute(
                    "DELETE FROM receipt_links WHERE paperless_id = ? AND txn_hash = ?",
                    (paperless_id, txn_hash),
                )
                raise
