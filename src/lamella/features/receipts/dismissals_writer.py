...
"""Writer for the ``custom "document-dismissed"`` directive.

Dismissals are part of the reconstruct contract: a user acknowledging
"this transaction never gets a document" is state, not cache. It has
to survive a DB delete. We stamp each dismissal as a directive in
``connector_links.bean`` (co-located with document links — all
document-related ledger state lives in one file).

Per ADR-0061 (ledger v4): the writer emits ``document-dismissed`` /
``document-dismissal-revoked`` only. The reader still accepts the
legacy ``receipt-dismissed`` / ``receipt-dismissal-revoked`` shapes
indefinitely; existing directives in a v3 ledger remain valid and
parseable. Mixed v3+v4 vocabulary in one ledger is supported by
design (the migration is opportunistic).
...
def append_dismissal(
    *,
    ...
    return append_custom_directive(
        ...
        directive_type=DIRECTIVE_DISMISSED_NEW,
        args=[txn_hash],
        meta=meta,
        run_check=run_check,
    )
...
def append_dismissal_revoke(
    *,
    ...
    return append_custom_directive(
        ...
        directive_type=DIRECTIVE_DISMISSAL_REVOKED_NEW,
        args=[txn_hash],
...
def read_dismissals_from_entries(entries) -> list[dict]:
    """Parse every dismissal directive in the loaded ledger entries
    (``document-dismissed`` and the legacy ``receipt-dismissed`` are
    both accepted) and filter by matching revokes. A dismissal is
    active iff the most recent dismissal/revoke directive for its
    txn_hash is a dismissal. Returns rows ready for upsert into the
    ``document_dismissals`` cache.
    """
    # Walk both directive types in load order; for each txn_hash, the
    # last entry wins. Both v3 (receipt-*) and v4 (document-*)
    # vocabularies are accepted per ADR-0061.
    state: dict[str, dict | None] = {}  # txn_hash -> row or None (revoked)
    for entry in entries:
        if not isinstance(entry, Custom):
            continue
        if entry.type not in DIRECTIVE_TYPES_ALL_DISMISSAL:
            continue
        txn_hash = custom_arg(entry, 0)
        if not isinstance(txn_hash, str) or not txn_hash:
            continue
        if entry.type in DIRECTIVE_TYPES_DISMISSAL_REVOKED:
            state[txn_hash] = None
            continue
