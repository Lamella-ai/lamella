...
"""Amazon Seller Central annual summary."""
...
def ingest_sheet(
    ...
    *,
    ...
    for idx, row in df.iterrows():
        row_num = int(idx) + 2
        date = parse_date(g(row, "Date/Time")) or parse_date(g(row, "Date"))
        total = safe_decimal(g(row, "Total"))
        if date is None and total is None:
            continue
        ttype = clean_str(g(row, "Type"))
        order_id = clean_str(g(row, "Order ID"))
        desc = clean_str(g(row, "Description")) or ttype
        ...
        insert_raw_row(
            ...
            payment_method="Amazon Seller Balance",
        )
        n += 1
    log.info("amazon_seller.ingest_sheet source_id=%d rows=%d", source_id, n)
    return n
