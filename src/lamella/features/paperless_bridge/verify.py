...
VISION_ALLOWED_MIMES: frozenset[str] = frozenset(
...
def _sniff_image_mime(blob: bytes, fallback: str | None = None) -> str:
    ...
    ``image/webp``. Falls back to ``image/jpeg`` when the bytes
    don't match a known signature — never returns the caller's
    fallback if it's outside the allowed set, since the API would
    just reject it anyway. Conversion of unsupported formats to a
    supported one is the caller's responsibility (see
    ``_to_vision_supported_image``).
    """
    if not blob:
        return "image/jpeg"
    head = blob[:16]
    ...
    if head[:4] == b"RIFF" and head[8:12] == b"WEBP":
        return "image/webp"
    fb = (fallback or "image/jpeg").split(";")[0].strip()
    return fb if fb in VISION_ALLOWED_MIMES else "image/jpeg"


def _to_vision_supported_image(
    blob: bytes, declared_mime: str | None,
) -> tuple[bytes, str]:
    """Coerce arbitrary image bytes to a vision-API-supported format.

    Anthropic's vision tier accepts only image/jpeg, image/png,
    image/gif, image/webp. iPhone HEIC, TIFF scans, BMP, JP2, and
    similar formats trigger HTTP 400:

      messages.0.content.0.image.source.base64.media_type:
      Input should be 'image/jpeg', 'image/png', 'image/gif' or
      'image/webp'

    When the byte signature already matches a supported format, this
    is a no-op — return the bytes as-is with the sniffed mime.
    Otherwise re-encode via Pillow to PNG (lossless, preserves text
    legibility for OCR-ish use cases like receipt verification).

    Returns the original bytes + sniffed mime when re-encoding
    fails, so the caller can still attempt the API call rather than
    silently dropping the document. The API will reject it with the
    same 400, but the audit log will reflect a single attempt rather
    than a hidden conversion failure.
    """
    sniffed = _sniff_image_mime(blob, declared_mime)
    if sniffed in VISION_ALLOWED_MIMES:
        # Trust the sniff: byte signature confirms one of the four
        # accepted formats.
        head = blob[:16]
        signature_ok = (
        ...
        if signature_ok:
            return blob, sniffed
    # Bytes don't match a supported signature. Re-encode via Pillow.
    try:
        from io import BytesIO
        from PIL import Image
    except Exception as exc:  # noqa: BLE001
        log.warning(
            "Pillow unavailable for vision-format coercion (%s); "
            "sending bytes as-is — API will likely reject.", exc,
        )
        return blob, sniffed
    try:
        img = Image.open(BytesIO(blob))
        # HEIC/HEIF needs pillow-heif; if not installed Pillow raises
        # UnidentifiedImageError. Fall through to the bare-bytes path.
        img.load()
    except Exception as exc:  # noqa: BLE001
        log.warning(
            "Pillow can't decode bytes (declared=%r, sniffed=%s): %s",
            declared_mime, sniffed, exc,
        )
        return blob, sniffed
    # Flatten alpha against white so JPEG-via-PNG converts cleanly.
    if img.mode in ("RGBA", "LA", "P"):
        img = img.convert("RGBA")
        from PIL import Image as _ImgMod
        bg = _ImgMod.new("RGB", img.size, (255, 255, 255))
        bg.paste(img, mask=img.split()[-1] if img.mode == "RGBA" else None)
        img = bg
    elif img.mode != "RGB":
        img = img.convert("RGB")
    out = BytesIO()
    # PNG keeps text crisp without lossy re-quantization. Vision
    # input cap is 1568px max edge per Anthropic's recommendation;
    # the caller already sized the PDF render path, but raw image
    # uploads from Paperless may be larger. Downscale here too.
    max_edge = 1568
    w, h = img.size
    edge = max(w, h)
    if edge > max_edge:
...
def _render_pdf_first_page_png(
    pdf_bytes: bytes, max_edge_px: int = 1568
) -> bytes | None:
    """Render page 1 of a PDF to a PNG sized for the vision API.

    Why: Paperless's ``/api/documents/{id}/thumb/`` returns a fixed
    ~500px JPEG and, for some documents (thumbnail generation failed,
    queued, or doc was just uploaded), serves a generic doc-shaped
    placeholder icon. Vision then describes the placeholder instead
    of the receipt. Since the verify pipeline already downloads the
    full PDF for classify_pdf_bytes, we can render page 1 ourselves
    at a vision-friendly resolution (Anthropic's recommended max
    edge is 1568px). Returns None when PyMuPDF is unavailable, the
    PDF is corrupt, or rendering fails — the caller falls back to
    download_thumbnail.
    """
    try:
        import pymupdf  # PyMuPDF — same dep as classify_pdf_bytes
    except ImportError:  # pragma: no cover — dep is core
        return None
    try:
        doc = pymupdf.open(stream=pdf_bytes, filetype="pdf")
    except Exception as exc:  # noqa: BLE001
        log.info("PDF render failed (corrupt?): %s", exc)
        return None
    try:
        if doc.page_count <= 0:
            return None
        ...
        scale = (max_edge_px / edge) if edge > 0 else 2.0
        # Cap upscaling: never enlarge tiny pages above 4x — they
        # rarely benefit and bloat the payload.
        if scale > 4.0:
            scale = 4.0
        if scale <= 0:
            scale = 2.0
        ...
        log.info("PDF page render failed: %s", exc)
        return None
    finally:
        try:
            doc.close()
...
class DocumentVerification(BaseModel):
    """Structured output from the document-verify model.
    ...
    Per ADR-0061 §5 this schema covers receipts, invoices, orders,
    statements, and tax forms. Type-conditional extraction is
    driven by the ``document_type`` parameter on the prompt
    builders; this schema carries the *union* of fields any type
    might emit. Receipt-only fields (subtotal/tax/tip) coexist
    with invoice-only fields (po_number/invoice_number/due_date)
    and order-only fields (order_number/ship_date) — all are
    optional and default to empty string when absent.

    The ``receipt_date`` field name is preserved verbatim despite
    the rename for two reasons: (1) cached AI decisions on disk
    deserialize cleanly without a key migration, and (2) the
    existing ``paperless_doc_index.receipt_date`` column is
    renamed to ``document_date`` by Worker A's Phase 2 migration,
    not by this schema.
    """
    ...
    # ADR-0061 §5: invoice-only fields. Empty string = not
    # extracted (or not applicable for this document_type). The
    # caller decides whether to surface these in the audit-note +
    # writeback path based on ``document_type``.
    po_number: str = ""
    invoice_number: str = ""
    due_date: str = ""               # ISO YYYY-MM-DD, or ""

    # ADR-0061 §5: order-only fields.
    order_number: str = ""
    ship_date: str = ""              # ISO YYYY-MM-DD, or ""
...
class VerifyOutcome:
    ...
    extracted: DocumentVerification | None = None
...
def _build_user_prompt(
    *,
    ...
    """Build the user prompt for the vision tier.

    ``document_type`` (ADR-0061 §5) selects type-conditional
    extraction sections appended below the common preamble. ``None``
    is treated as ``"receipt"`` for backwards compatibility with
    callers that haven't been updated to thread the type through
    yet — the legacy receipt-shaped prompt was the implicit default
    before Phase 3.
    """
    doc_label = _document_type_label(document_type)
    lines: list[str] = []
    lines.append(
        f"Re-extract the structured fields from the attached "
        f"{doc_label} image."
    )
    lines.append("")
    lines.append(
        f"Current Paperless fields (may be wrong — verify against the image):"
    )
    ...
    if source_type == "ocr_pdf":
        ...
        lines.append("")
    # Type-conditional return-shape spec (ADR-0061 §5).
    lines.append(_return_fields_spec(document_type))
    if page_count <= 1:
        lines.append(
            f"corrected_content: a clean plaintext re-transcription of "
            f"EVERYTHING visible on the {doc_label} (header, line items, "
            f"totals, footer, payment info) — one line per logical "
            f"row. This REPLACES the existing OCR in Paperless when we "
            f"push corrections back. If the image is unreadable, leave "
            f"it as an empty string and we won't touch the stored OCR."
        )
    else:
        lines.append(
...
def _document_type_label(document_type: str | None) -> str:
    """Human-readable noun for the prompt body. ``None`` → "receipt"
    for backwards compatibility with pre-Phase-3 callers (ADR-0061
    §5)."""
    canonical = (document_type or "receipt").strip().lower()
    if canonical in {"receipt", "invoice", "order", "statement", "tax"}:
        return canonical
    # Fall back to "document" for "other" / unrecognized values so
    # the prompt never names a type the schema doesn't recognise.
    return "document"


def _return_fields_spec(document_type: str | None) -> str:
    """Return the JSON-shape directive line for the prompt, gated
    on ``document_type``. All types share ``vendor``, ``total``,
    ``receipt_date`` (the document date), ``confidence``,
    ``ocr_errors_noted``, ``reasoning``, ``corrected_content``;
    type-specific monetary / identifier fields are appended only
    for the types that need them.

    Receipt is the implicit default (``document_type=None`` falls
    through to the legacy receipt shape) so callers that don't
    yet thread the type through still produce the same prompt
    text the system was emitting before Phase 3.
    """
    canonical = (document_type or "receipt").strip().lower()
    common = (
        "Return a JSON object with: receipt_date (YYYY-MM-DD, the "
        "primary date on the document), vendor, total, "
        "confidence{field: 0..1}, ocr_errors_noted[], reasoning, "
        "corrected_content"
    )
    if canonical == "receipt":
        return (
            f"{common}, plus the receipt-specific fields: "
            f"subtotal, tax, tip, line_items[]."
        )
    if canonical == "invoice":
        return (
            f"{common}, plus the invoice-specific fields: "
            f"po_number, invoice_number, due_date (YYYY-MM-DD), "
            f"line_items[]."
        )
    if canonical == "order":
        return (
            f"{common}, plus the order-specific fields: "
            f"order_number, ship_date (YYYY-MM-DD), line_items[]."
        )
    if canonical in {"statement", "tax"}:
        # Statements and tax forms are explicitly excluded from the
        # auto-link path (see ``RECEIPT_EXCLUDED_DOCTYPE_PATTERNS`` →
        # ``document_type`` discriminator in ``txn_matcher.py``).
        # Extract only the minimal identification fields — no
        # subtotal/tax/tip/po_number, since those don't apply to
        # periodic-summary docs and prompting for them produces
        # confident-but-wrong values culled from per-line entries.
        return (
            f"{common}. This is a {canonical} document — extract only "
            f"the fields above and leave subtotal / tax / tip / "
            f"po_number / invoice_number / order_number empty. "
            f"Statements and tax forms are not auto-linked to "
            f"transactions."
        )
    # Unknown / "other" — fall back to the receipt shape so the
    # caller still receives a useful extraction.
    return (
        f"{common}, plus subtotal, tax, tip, line_items[]."
    )
...
def _build_ocr_text_prompt(
    *,
    ...
    ``document_type`` (ADR-0061 §5) selects type-conditional return
    sections; ``None`` means "treat as receipt" for backwards
    compat with callers not yet updated.
    """
    doc_label = _document_type_label(document_type)
    lines: list[str] = []
    lines.append(
        f"Extract structured {doc_label} fields from the OCR text "
        f"below. This was OCR'd by Paperless from a scanned or "
        f"photographed {doc_label}. It is USUALLY accurate for clear "
        f"{doc_label}s but can be garbled on blurry / thermal-faded / "
        f"crumpled ones."
    )
    lines.append("")
    ...
    if hypothesis and (
        ...
        lines.append(
            f"GROUND TRUTH from the linked transaction — this is what "
            f"the user already KNOWS about this {doc_label} (bank "
            f"statement / SimpleFIN row). Your job is to corroborate, "
            f"not to overrule. If the OCR vendor disagrees with the "
            f"merchant below, the OCR is more likely wrong than this "
            f"hypothesis (stylized logos and branding don't always come "
            f"through OCR). When that happens, add 'NEEDS_VISION' to "
            f"ocr_errors_noted so the caller escalates to a vision "
            f"pass against the image."
        )
        if hypothesis.suspected_date:
        ...
        lines.append("")
    # Type-conditional return-shape spec (ADR-0061 §5). Strip the
    # ``corrected_content`` clause from the common spec — text-only
    # tier never re-extracts content.
    lines.append(_return_fields_spec(document_type))
    lines.append(
...
def _ocr_extraction_quality(
    ...
    *,
    ...
    fields_of_interest: tuple[str, ...] | None = None,
    ...
    ``fields_of_interest`` narrows escalation to a subset of canonical
    fields. When set: the average is computed only across those fields,
    and the vendor-mismatch escalation is suppressed unless ``vendor``
    is in the set. Use case: a date-only verify ($-cheap path bound to
    Lamella:DateAnomaly) shouldn't escalate to the vision tier just
    because the OCR vendor or total came back blurry — those fields
    aren't in scope for this run. Pass ``None`` (default) for the
    legacy "all canonical fields" behavior.
    """
    ...
    all_vals: dict[str, float] = {
        "receipt_date": float(conf.receipt_date),
        "vendor": float(conf.vendor),
        "total": float(conf.total),
        "subtotal": float(conf.subtotal),
        "tax": float(conf.tax),
    }
    if fields_of_interest:
        vals = [all_vals[k] for k in fields_of_interest if k in all_vals]
    else:
    ...
    # Vendor-mismatch escalation only fires when vendor is in scope —
    # a date-only verify shouldn't pay for vision because of a vendor
    # that the user didn't ask us to check.
    vendor_in_scope = (
        fields_of_interest is None or "vendor" in fields_of_interest
    )
    if vendor_in_scope and hypothesis and hypothesis.suspected_vendor:
...
class VerifyService:
    ...
    async def verify_and_correct(
        ...
        *,
        ...
        ``document_type`` (ADR-0061 §5) selects type-conditional
        extraction sections in the AI prompt. ``None`` means
        "treat as receipt" — the legacy default — for backwards
        compatibility with callers not yet updated. The orchestrator
        can pull this from ``paperless_doc_index.document_type``
        once Worker A's Phase 2 migration lands; until then this
        stays at the call-site default.
        ...
        page_count = 1
        ocr_candidate: DocumentVerification | None = None
        ...
        if extracted is None:
            ...
            if fetch_result is None:
                ...
                # Safety net: if vision came back empty (placeholder
                # image, low-quality scan, model refusal) but Tier 1
                # produced something usable, prefer per-field whichever
                # has the higher confidence. Without this, a vision
                # bust drops Tier 1's high-confidence vendor/total/date
                # on the floor and the only thing that lands on the doc
                # is the Lamella Fixed tag — exactly the "tagged but
                # nothing changed" symptom.
                if ocr_candidate is not None:
                    extracted = _merge_extractions(extracted, ocr_candidate)
                    decision_id = decision_id or ocr_decision_id
                    if extracted is not result.data:
                        emit(
                            "Merged Tier 1 OCR extraction into vision "
                            "result for fields where OCR was more "
                            "confident.",
                            "info",
                        )
        ...
        # already have (no schema change to DocumentVerification):
        ...
        # enough, and whose actual value changed. When the caller
        # narrowed scope via fields_of_interest, the patch is further
        # restricted so a date-only verify cannot accidentally PATCH
        # other fields the model coughed up incidentally.
    ...
    def _load_index_row(self, paperless_id: int) -> sqlite3.Row | None:
        ...
        # ADR-0061 Phase 2: receipt_date column renamed to document_date.
        # Aliased back so the downstream `_get("receipt_date")` reader
        # (and the rest of verify.py's prompt/diff machinery owned by
        # Worker B's Phase-3 scope) keeps the same dict-key surface.
        return self.conn.execute(
            """
            SELECT paperless_id, title, vendor, total_amount,
                   subtotal_amount, tax_amount, document_date AS receipt_date,
    ...
    async def _extract_from_ocr_text(
        ...
        *,
        ...
        hypothesis: VerifyHypothesis | None,
        document_type: str | None = None,
    ) -> tuple[tuple[DocumentVerification, int] | None, str | None]:
        ...
        ``document_type`` selects type-conditional prompt sections
        per ADR-0061 §5; ``None`` falls back to the receipt-shaped
        prompt for backwards compat.
        """
        client = self.ai.new_client()
        if client is None:
    ...
    async def _fetch_image_for_vision(
        ...
        if source_type == "image":
            ...
            # iPhone HEIC, scanner TIFF, BMP, JP2, etc. are all valid
            # uploads to Paperless but get rejected by Anthropic's
            # vision tier with HTTP 400. Coerce to a supported format
            # (PNG via Pillow) before returning.
            coerced, coerced_mime = _to_vision_supported_image(bytes_, mime)
            return coerced, coerced_mime, 1
        ...
        if pdf_bytes:
            ...
            if classified == "native_pdf":
                return None
            # Render page 1 ourselves rather than fetching Paperless's
            # /thumb/ endpoint. The thumbnail is fixed-size and, for
            # some docs, returns a generic placeholder icon — vision
            # then describes the placeholder instead of the receipt.
            png_bytes = _render_pdf_first_page_png(pdf_bytes)
            if png_bytes:
                return png_bytes, "image/png", page_count
            log.info(
                "PyMuPDF render returned no bytes for %d; falling back "
                "to Paperless thumbnail",
                paperless_id,
            )
        thumb_bytes, ctype = await self.paperless.download_thumbnail(paperless_id)
        coerced, coerced_mime = _to_vision_supported_image(thumb_bytes, ctype)
        return coerced, coerced_mime, page_count
    ...
    def _mirror_patch_to_index(
        ...
        *,
        ...
        if "receipt_date" in patch and patch["receipt_date"]:
            ...
            # we also expose the canonical document date separately for
            # the custom field, so update both when the source agrees.
            # ADR-0061 Phase 2: SQL column renamed receipt_date →
            # document_date; the Python ``patch`` dict key stays
            # ``receipt_date`` (Worker B's Phase-3 scope).
            cols.append("document_date")
    ...
    def _update_local_index(
        ...
        *, corrected_content: str | None = None,
        ...
        sets: list[str] = []
        args: list[Any] = []
        # ADR-0061 Phase 2: SQL column renamed receipt_date → document_date.
        # The patch dict key (left-hand side) stays `receipt_date` (Worker B's
        # Phase-3 surface); the SQL column (right-hand side) follows the rename.
        mapping = {
...
def _merge_extractions(
    primary: DocumentVerification, fallback: DocumentVerification,
) -> DocumentVerification:
    """For each scalar field, take the value+confidence with the
    higher confidence. Returns ``primary`` unchanged when no fallback
    field beats primary (so the caller can detect a no-op via
    identity check). Reasoning / ocr_errors_noted / corrected_content
    stay with the primary (they describe how it was extracted, not
    what's authoritative).
    """
    fields = ("receipt_date", "vendor", "total", "subtotal", "tax", "tip")
    p_conf = primary.confidence
    f_conf = fallback.confidence
    needs_merge = False
    overrides: dict[str, str] = {}
    conf_overrides: dict[str, float] = {}
    for fn in fields:
        pv = getattr(primary, fn, "") or ""
        fv = getattr(fallback, fn, "") or ""
        pc = float(getattr(p_conf, fn, 0.0) or 0.0)
        fc = float(getattr(f_conf, fn, 0.0) or 0.0)
        if not fv:
            continue
        # Take fallback if it has a value and either primary lacks
        # one or fallback's confidence beats primary's.
        if not pv or fc > pc:
            overrides[fn] = fv
            conf_overrides[fn] = fc
            needs_merge = True
    if not needs_merge:
        return primary
    # Build a merged copy. Pydantic v2 ``model_copy(update=…)``.
    new_conf = p_conf.model_copy(update=conf_overrides)
    return primary.model_copy(update={**overrides, "confidence": new_conf})


def _compute_diffs(
    current: dict[str, Any], extracted: DocumentVerification,
) -> list[VerifyDiff]:
    conf = extracted.confidence
    diffs: list[VerifyDiff] = []
    ...
    def _add(field_name: str, extracted_val: str, field_conf: float):
        ...
        # every scalar field on DocumentVerification is a plain str.
...
def _suggest_title(extracted: DocumentVerification) -> str:
...
def _fields_to_patch(
    ...
    *,
    ...
    ``fields_of_interest`` restricts which canonical fields can land
    in the patch. Vendor-driven correspondent and title side-effects
    are also gated: they only apply when ``vendor`` is in scope. Pass
    ``None`` (default) to allow all canonical fields — the legacy
    behavior. Use case: a date-only verify must not write back vendor
    or correspondent corrections even if Tier 1 happened to extract
    them confidently.
    """
    foi = set(fields_of_interest) if fields_of_interest else None
    ...
    for field_name, canonical_val in (
        ...
        if foi is not None and field_name not in foi:
            continue
    ...
    # correspondent too. Skipped entirely when vendor is out of
    # scope (date-only / total-only verifies).
    vendor_in_scope = foi is None or "vendor" in foi
    if vendor_in_scope:
    ...
    # vendor stay untouched. Title rewrites are vendor-driven and
    # therefore also skipped when vendor is out of scope.
    if vendor_in_scope and _title_needs_rewrite(
        current_title, extracted, diffs, threshold,
    ):
