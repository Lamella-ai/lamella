...
def _guess_role(name: str) -> str:
    ...
    if not n:
        return "ignore"
    # Lamella_* writeback fields take precedence over keyword matches
    # so e.g. ``Lamella_Account`` doesn't get tagged ``payment_last_four``
    # by the "card" / "account" heuristics below. The matcher writer
    # creates these on first writeback and they have no read-side role.
    if raw.startswith("Lamella_") or n.startswith("lamella_"):
