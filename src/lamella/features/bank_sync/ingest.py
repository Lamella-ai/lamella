...
def _fixme_for_entity(
    ...
    Per the SimpleFIN convention (positive = credit to the source
    account), a positive amount is money INTO the account — a deposit
    on a bank account or a payment received on a credit card — which
    belongs in Income's box. A non-positive amount is money out, which
    belongs in Expenses's box.
    ...
    Expenses-only routing. New ingest call sites pass the signed
    PendingEntry/SimpleFIN amount.
    """
    root = "Expenses"
...
class SimpleFINIngest:
    ...
    async def _classify(
        ...
        *,
        ...
        # Sign-aware: deposits route to Income:{entity}:FIXME so the
        # AI sees the Income whitelist; withdrawals route to
        # Expenses:{entity}:FIXME. Without this, every $X positive
        # amount on checking ends up classified as a fee/expense
        # because the AI's per-root whitelist gives it nothing else
        # to pick from.
        fixme_account = _fixme_for_entity(entity, Decimal(txn.amount))
    ...
    async def _maybe_ai_classify(
        ...
        *,
        ...
        # (stored in ``receipt_links.txn_hash``), but that lookup was
        ...
        try:
            ...
            from lamella.features.ai_cascade.receipt_context import fetch_receipt_context
            ...
            #   1. Linked — ``receipt_links.txn_hash`` carries either
            ...
            try:
                receipt_ctx = fetch_receipt_context(
                    self.conn,
