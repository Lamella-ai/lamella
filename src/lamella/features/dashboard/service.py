...
def activity_summary(conn: sqlite3.Connection) -> dict[str, int]:
    ...
    Semantics:
      ...
      - ``receipts_linked_today``: receipt_links rows whose
    ...
    receipts_today = conn.execute(
        "SELECT COUNT(*) AS n FROM receipt_links WHERE DATE(linked_at) = ?",
        (today_iso,),
    ).fetchone()["n"]
...
def compute_expenses(
    ...
    *, exclude_fixme: bool = True,
    ...
    return _sum_postings(
        entries, slug, root="Expenses", start=start, end=end,
        exclude_fixme_txns=exclude_fixme,
    )
...
def _expense_rollup_by_account(
    ...
    for entry in _entity_txns(entries, slug):
        ...
        for p in entry.postings:
            ...
            if p.units is None or p.units.number is None:
                continue
            out[acct] += Decimal(p.units.number)
    return dict(out)
    ...
    def _build() -> dict:
        ...
        for entry in _entity_txns(entries, slug):
            ...
            for p in entry.postings:
                ...
                if p.units is None or p.units.number is None:
                    continue
                totals[payee] += Decimal(p.units.number)
            counts[payee] += 1
