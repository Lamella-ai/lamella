...
class MileageService:
    ...
    def neighbor_before(
        self, vehicle: str, entry_date: date_t, odometer_end: int,
    ) -> MileageRow | None:
        """Nearest odometer-bearing entry at-or-before the insertion
        point in the (entry_date, odometer_end) ordering. Entries
        carry dates, not reliable times — within a day the odometer
        IS the timeline, so a backdated reading slots between two
        same-day readings by value, not by when it was typed in."""
        row = self.conn.execute(
            """
            SELECT id, entry_date, entry_time, vehicle, vehicle_slug,
                   odometer_start, odometer_end, miles,
             ...
             WHERE vehicle = ? AND odometer_end IS NOT NULL
               AND (entry_date < ?
                    OR (entry_date = ? AND odometer_end <= ?))
          ORDER BY entry_date DESC, odometer_end DESC, id DESC
             LIMIT 1
            """,
            (vehicle, entry_date.isoformat(), entry_date.isoformat(),
             int(odometer_end)),
        ).fetchone()
        return _row_to_mileage(row) if row else None
    ...
    def neighbor_after(
        ...
        """Nearest odometer-bearing entry after the insertion point in
        the (entry_date, odometer_end) ordering — the entry a
        backdated reading must stay below (and whose chained start
        may need re-anchoring). Returns the raw row because the
        caller needs the DB id."""
        return self.conn.execute(
            """
            SELECT id, entry_date, odometer_start, odometer_end, miles
              FROM mileage_entries
             WHERE vehicle = ? AND odometer_end IS NOT NULL
               AND (entry_date > ?
                    OR (entry_date = ? AND odometer_end > ?))
          ORDER BY entry_date ASC, odometer_end ASC, id ASC
             LIMIT 1
            """,
            (vehicle, entry_date.isoformat(), entry_date.isoformat(),
             int(odometer_end)),
    ...
    def add_entry(
        ...
        *,
        ...
        if odometer_start is not None and odometer_end is not None:
            ...
            # No explicit start — derive it from the nearest reading
            # at-or-before (entry_date, odometer_end). Anchoring to
            # the date-aware neighbor (not the globally-latest entry)
            # is what makes backdating work: with 1000 logged Jan 1
            # and 1200 Jan 3, a Jan 2 reading of 1100 slots between
            # them instead of erroring against the 1200.
            odo_end = int(odometer_end)
            prev = self.neighbor_before(vehicle, entry_date, odo_end)
            if prev is None or prev.odometer_end is None:
                # Nothing on or before this point in the chain. Either
                # a true first trip, or a backdated value below every
                # same-day reading — surface the right message.
                nxt0 = self.neighbor_after(vehicle, entry_date, odo_end)
                if nxt0 is not None and str(nxt0["entry_date"])[:10] == entry_date.isoformat():
                    raise MileageValidationError(
                        f"odometer {odo_end} on {entry_date} is below the "
                        f"{int(nxt0['odometer_end'])} reading already logged "
                        f"that day — a reading can't go backwards. Check the "
                        "date or the value."
                    )
                raise MileageValidationError(
                    f"no odometer reading on or before {entry_date} for "
                    f"{vehicle!r}; enter start odometer or miles directly "
                    "for the first trip"
                )
            if odo_end < int(prev.odometer_end):
                raise MileageValidationError(
                    f"odometer {odo_end} on {entry_date} is below the "
                    f"{int(prev.odometer_end)} reading logged on "
                    f"{prev.entry_date} — a reading can't go backwards. "
                    "Check the date or the value."
                )
            nxt = self.neighbor_after(vehicle, entry_date, odo_end)
            if nxt is not None:
                nxt_end = int(nxt["odometer_end"])
                if nxt_end < odo_end:
                    raise MileageValidationError(
                        f"odometer {odo_end} on {entry_date} is above the "
                        f"{nxt_end} reading logged on {nxt['entry_date']} — "
                        "a backdated reading must stay below every later "
                        "one. Check the date or the value."
                    )
                nxt_start = nxt["odometer_start"]
                if nxt_start is not None and int(nxt_start) < odo_end:
                    if int(nxt_start) == int(prev.odometer_end):
                        # The next trip's start was chained to prev's
                        # end. Splice this reading into the chain:
                        # re-anchor the next trip to start where the
                        # inserted one ends so the miles between prev
                        # and nxt aren't double-counted.
                        self.conn.execute(
                            "UPDATE mileage_entries "
                            "   SET odometer_start = ?, miles = ? "
                            " WHERE id = ?",
                            (odo_end, float(nxt_end - odo_end),
                             int(nxt["id"])),
                        )
                    else:
                        raise MileageValidationError(
                            f"odometer {odo_end} on {entry_date} falls "
                            f"inside the {int(nxt_start)}→{nxt_end} trip "
                            f"logged on {nxt['entry_date']} — those miles "
                            "are already counted. Edit that trip first if "
                            "this reading is correct."
                        )
            odo_start = int(prev.odometer_end)
            derived_miles = float(odo_end - odo_start)
    ...
    def write_import_rows(
        ...
        *,
        ...
        for row in rows:
            ...
            duplicate_reason = self._existing_row_conflict_reason(
                entry_date=row.entry_date,
            ...
            if duplicate_reason is not None:
                skipped += 1
                messages.append(
                    f"line {row.line_no}: {duplicate_reason}"
                )
                continue
    ...
    def mark_existing_duplicates(
        ...
        *,
        ...
        """Annotate preview rows that already exist in mileage_entries.

        Rows marked here are still returned in-order so the preview can
        show a warning before commit; commit then skips them via the same
        duplicate predicate in ``write_import_rows``.
        """
        out: list[ImportPreviewRow] = []
        for row in rows:
            if row.error is not None or row.entry_date is None or row.miles is None:
                out.append(row)
                continue
            duplicate_reason = self._existing_row_conflict_reason(
                entry_date=row.entry_date,
            ...
            if duplicate_reason is not None:
                if row.conflict is None:
                    row = replace(
                        row,
                        conflict=f"{duplicate_reason} (will be skipped on import)",
                    )
            out.append(row)
        return out

    def _existing_row_conflict_reason(
        self, *,
        entry_date: date_t,
        entry_time: str | None,
        vehicle: str,
        ...
        if self._overlapping_range_exists(
            entry_date=entry_date,
            vehicle=vehicle,
            odometer_start=odometer_start,
            odometer_end=odometer_end,
        ):
            return "overlapping odometer range — would double count miles"
        return None
    ...
    def _overlapping_range_exists(
        ...
        """Detect date+vehicle rows whose odometer ranges overlap.

        This catches accidental nested trips on the same day, e.g.
        existing 1000→1100 and incoming 1050→1075.
        """
        if odometer_start is None or odometer_end is None:
            return False
        lo = min(int(odometer_start), int(odometer_end))
        hi = max(int(odometer_start), int(odometer_end))
        # Zero-mile marker rows (start == end) are informational
        # substantiation notes. They don't consume range and should be
        # allowed even if they sit inside another trip span.
        if lo == hi:
            return False
        rows = self.conn.execute(
            """
        ...
        for r in rows:
            if r["odometer_start"] is None or r["odometer_end"] is None:
                continue
            existing_lo = min(int(r["odometer_start"]), int(r["odometer_end"]))
            existing_hi = max(int(r["odometer_start"]), int(r["odometer_end"]))
            if existing_lo == existing_hi:
                continue
