...
def _receipt_for_txn(
    ...
    *,
    ...
    ``receipt_links.txn_hash`` is the Beancount content hash, which
    ...
    2. In-place rewrite or staged-txn promotion — same entry edited,
       ...
       fallback against ``receipt_links.txn_date`` + ``txn_amount``.
    ...
    row = conn.execute(
        f"SELECT paperless_id FROM receipt_links "
    ...
    fuzzy = conn.execute(
        "SELECT paperless_id FROM receipt_links "
        "WHERE txn_date = ? "
