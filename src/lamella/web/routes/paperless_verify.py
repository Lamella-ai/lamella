...
def _hypothesis_from_link(
    ...
    """
    row = conn.execute(
        "SELECT txn_hash, txn_date, txn_amount FROM receipt_links "
...
@router.post("/paperless/{doc_id}/verify", response_class=HTMLResponse)
...
@router.post("/paperless/{doc_id}/verify/sync", response_class=HTMLResponse)
...
@router.post("/paperless/{doc_id}/enrich", response_class=HTMLResponse)
