...
def general_settings_page(
    ...
    else (number formatting, timezone). Lives at its own route per
    the dashboard pattern; the /settings landing only links here, no
    inline form."""
    # Resolve current app_tz: store override wins over Settings default.
    current_app_tz = store.get("app_tz") or settings.app_tz or "UTC"
    return request.app.state.templates.TemplateResponse(
...
def general_settings_save(
    ...
    app_tz: str | None = Form(default=None),
    settings: Settings = Depends(get_settings),
    store: AppSettingsStore = Depends(get_app_settings_store),
):
    ...
    # Timezone — validate via zoneinfo before storing.
    if app_tz is not None:
        ...
        if v:
            try:
                ...
                store.set("app_tz", v)
                try:
                    settings.app_tz = v  # type: ignore[attr-defined]
                except Exception:  # noqa: BLE001
                    pass
            except zoneinfo.ZoneInfoNotFoundError:
                pass  # Silently discard invalid IANA names.
