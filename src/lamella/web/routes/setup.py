...
def _auto_scaffold_entity_chart(
    *,
    conn,
    settings: Settings,
    slug: str,
) -> None:
    """Open the canonical chart for ``slug`` when available.

    Best-effort helper for setup entity-create flows. If the entity's
    type/schedule doesn't map to a chart yet, this no-ops.
    """
    ...
    # Keep this idempotent and "unique": preserve order but never pass
    # duplicate account paths into write_opens.
    paths = list(dict.fromkeys(c["path"] for c in candidates))
    if not paths:
        return
    ...
    writer.write_opens(
        paths,
        comment=f"Auto-scaffold for {slug} ({len(paths)} accounts)",
    )
...
async def setup_entity_add_person(
    ...
    try:
        _auto_scaffold_entity_chart(conn=conn, settings=settings, slug=slug)
    except Exception as exc:  # noqa: BLE001
        log.warning("auto-scaffold for %s skipped: %s", slug, exc)
    request.app.state.setup_required_complete = False
