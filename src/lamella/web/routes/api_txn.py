...
def _do_ledger_classify(
    *,
    ...
    # Same pre-write guards the staged classify path runs: validate
    # the path shape (ADR-0045) and the entity-first rule (ADR-0042),
    # auto-scaffold a target that deepens an existing branch, and
    # confirm the Open covers the txn date. Without these, a brand-new
    # path (e.g. the user typing Income:personal:GiftReceived over the
    # AI's proposal) fails post-write with an opaque bean-check error
    # instead of either succeeding or returning a clean 400.
    ...
    try:
        validate_beancount_account(target_account)
        _entity_slugs = frozenset(
            r["slug"] for r in conn.execute("SELECT slug FROM entities")
        )
        validate_entity_first_path(target_account, _entity_slugs)
    except InvalidAccountSegmentError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    ext_err = ensure_target_account_open(
        reader, settings, target_account, txn_date,
    )
    if ext_err:
        raise HTTPException(status_code=400, detail=ext_err)
    open_err = check_account_open_on(
        reader, target_account, txn_date, settings=settings,
    )
    if open_err:
