    ...
    def _worker(ctx_job):
        ...
        try:
            if parsed.is_staged:
                ...
                # Sign convention is account-kind dependent. For a
                # bank account (asset) a positive amount is money IN
                # (deposit). For a credit card / line of credit /
                # loan / mortgage (liability) a positive amount is a
                # CHARGE — the user spending — and a negative amount
                # is the bill payment. The deposit short-circuit is
                # meant to skip AI for "money in" only; misfiring on
                # credit-card charges silently kills AI classification
                # for the most common case. Resolve the source account
                # kind and flip the sign comparison for liabilities.
                _is_liability_kind = False
                if _row is not None:
                    try:
                        from lamella.web.routes.staging_review import (
                            _resolve_account_path as _rap,
                            _resolve_card_kind as _rck,
                        )
                        _src_acct = _rap(conn, _row.source, _row.source_ref)
                        _kind = _rck(conn, _src_acct) if _src_acct else None
                        if _kind in (
                            "credit_card", "line_of_credit",
                            "loan", "mortgage",
                        ):
                            _is_liability_kind = True
                    except Exception:  # noqa: BLE001
                        _is_liability_kind = False
                if _is_liability_kind:
                    # On a liability, "money in" to the account =
                    # bill payment = negative amount.
                    _is_deposit = _amt < 0
                else:
                    _is_deposit = _amt > 0
            else:
                # Ledger txn: read the FIXME-leg amount from the
                # parsed entry. Convention: FIXME-leg is the offset
                # of the bank-side, so a NEGATIVE FIXME amount means
                # money IN to the bank-side account (deposit on an
                # asset, OR a charge on a liability — same sign,
                # different meaning). Resolve the bank-side account's
                # kind to flip the sign comparison on liabilities.
                ...
                for e in reader.load().entries:
                    ...
                    for p in e.postings or ():
                        ...
                        elif acct and not _bank_acct and p.units:
                            _bank_acct = acct
                    if _fixme_amt is not None:
                        try:
                            ...
                            _kind = _rck(conn, _bank_acct) if _bank_acct else None
                        except Exception:  # noqa: BLE001
                            _kind = None
                        if _kind in (
                            "credit_card", "line_of_credit",
                            ...
                            # Liability bank-side: a positive FIXME
                            # amount means money IN to the liability =
                            # bill payment. Flip the convention.
