...
def _annotate_receipts(
    ...
    # ADR-0061 Phase 5: column + dict key are ``document_date`` end-to-end.
    rows = conn.execute(
        f"""
    ...
    for h in hits:
        ...
        try:
            if row["document_date"] and row["txn_date"]:
                d1 = date.fromisoformat(str(row["document_date"])[:10])
...
def txn_detail(
    ...
    # ``document_links.txn_hash`` is overloaded — it stores either the
    # Beancount content hash (legacy + matcher-sweep links) or the
    # UUIDv7 lamella-txn-id token (links written via
    # /txn/{token}/document-link). Query both so a document linked
    ...
    # ``document_date``; the legacy ``receipt_date`` alias is gone.
    receipt_rows = conn.execute(
        ...
        SELECT rl.paperless_id, rl.match_method, rl.match_confidence,
               ...
               pdi.document_date, pdi.created_date, pdi.content_excerpt,
...
def search_receipt_hunt_result(
    ...
    this handler fetches it and hands it to document_hunt_result.html.
    ...
    return request.app.state.templates.TemplateResponse(
        request, "document_hunt_result.html",
        {"report": report},
    )
...
_PALETTE_PAGES = [
    ...
    {"label": "Receipts", "href": "/documents", "kind": "page"},
    {"label": "Receipts needed", "href": "/documents/needed", "kind": "page"},
