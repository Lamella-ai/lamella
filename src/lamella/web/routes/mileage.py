...
def _render_page(
    ...
    *,
    ...
    ctx = {
        ...
        # ADR-0066: on a validation error the submitted values come
        # back so the New-trip form re-renders filled in.
...
def _render_edit_page(
    ...
    *,
    ...
    """Render the per-trip edit page. On a validation error the
    caller passes the submission back via ``form_values`` so the form
    re-renders with the user's edits intact (ADR-0066)."""
    return request.app.state.templates.TemplateResponse(
...
def mileage_edit_form(
    ...
    return _render_edit_page(request, conn=conn, reader=reader, row=row)
...
async def mileage_edit_submit(
    ...
    def _err(message: str) -> HTMLResponse:
        # ADR-0066: re-render with the user's edits, never a redirect
        # that resets the form back to the stored row.
        return _render_edit_page(
            request, conn=conn, reader=reader, row=row,
            error=message,
    ...
    try:
        ...
        return _err(f"Invalid miles value {miles_raw!r}.")
    if miles_val < 0:
        return _err("Miles can't be negative.")
    ...
    if odo_start_v is not None and odo_end_v is not None:
        if odo_end_v < odo_start_v:
            return _err(
                f"End odometer ({odo_end_v}) must be at or above start "
                f"odometer ({odo_start_v})."
            )
...
def _render_quick_page(
    ...
    *,
    ...
    """Render the quick-log page. On a validation error the caller
    passes the submitted values back via ``form_values`` so the form
    re-renders filled-in — losing a typed-out trip note to a fat-
    fingered odometer is the bug this exists to prevent."""
    vehicles = _known_vehicles(conn)
    last_slug = _quick_get_last_vehicle_slug(conn) or _most_recent_vehicle_slug(conn)
    last_name = next(
...
def mileage_quick_page(
    ...
    return _render_quick_page(request, conn)
...
def mileage_quick_submit(
    ...
    (no redirect) — a redirect-to-fresh-GET wipes everything the user
    typed, which is exactly wrong when the fix is a one-character
    odometer edit. See ADR-0066."""
    submitted = {
    ...
    def _err(message: str) -> HTMLResponse:
        return _render_quick_page(
            request, conn,
            error=message, form_values=submitted, status_code=422,
        )
    ...
    try:
        ...
        return _err(f"Invalid odometer value {odometer_end!r}.")
    cat = (category or "").strip().lower() or "business"
    if cat not in {"business", "commuting", "personal"}:
        return _err(f"Invalid purpose {category!r}.")
    ...
    if vrow is None:
        return _err(
            f"Unknown vehicle {vehicle_slug!r} — pick one from the list."
        )
...
def mileage_rates_page(
    ...
    return _render_rates_page(request, settings=settings, conn=conn)
...
def mileage_rate_add(
    ...
    try:
        ...
        # ADR-0066: re-render with the submission instead of the old
        # ?error=bad_input redirect (which the template never even
        # displayed).
        return _render_rates_page(
            request, settings=settings, conn=conn,
            error=(
                "Invalid input — effective-from must be YYYY-MM-DD and "
                "rate a decimal like 0.67."
            ),
            form_values={
...
def create_mileage_entry(
    ...
    service = _service(settings, conn)
    # ADR-0066: hold the raw submission so every validation error can
    ...
    def _err(message: str) -> HTMLResponse:
        return _error_response(
            ...
            message=message, form_values=submitted,
        )
    ...
    if odometer_start is not None and odometer_start.strip():
        try:
            ...
            return _err(f"invalid start odometer value {odometer_start!r}")
    ...
    if miles is not None and miles.strip():
        try:
            ...
            return _err(f"invalid miles value {miles!r}")
    ...
    for raw, name, target in (
        ...
        if raw and raw.strip():
            try:
                ...
                return _err(f"invalid {name} value {raw!r}")
            if target == "biz_val":
                biz_val = v
    ...
    if cat_val is not None and cat_val not in {
        ...
        return _err(f"invalid category {category!r}")
...
def _error_response(
    ...
    *,
    ...
    if _is_htmx(request):
        # HTMX path: 400 + toast, no swap — the form keeps its DOM
        # state, so the user's input survives without server help.
        toast = (
    ...
    # Vanilla path: re-render with the submitted values (ADR-0066) —
    # a redirect-to-fresh-GET here would clear everything typed.
    return _render_page(
        request, settings=settings, conn=conn, reader=reader, store=store,
