...
def save_entity(
    ...
    # Auto-scaffold canonical chart paths unconditionally. Skipping is
    # not optional: the next setup-recovery sweep would flag the entity
    # as missing accounts anyway, so always doing it now avoids the
    # detour. Idempotent: AccountsWriter only opens missing paths.
    try:
        ...
        if schedule_yaml:
            ...
            wanted = list(dict.fromkeys(c["path"] for c in candidates))
            existing = {
                e.account for e in reader.load().entries
                if hasattr(e, "account")
            }
            paths = [p for p in wanted if p not in existing]
            if paths:
...
def scaffold_preview(
    ...
    # /accounts/{path} only resolves for paths registered in
    # accounts_meta (seed deliberately skips Expense/Income trees) —
    # "manage →" must not link there for unregistered paths or it 404s.
    meta_paths = {
        r["account_path"]
        for r in conn.execute("SELECT account_path FROM accounts_meta")
    }
    for c in candidates:
        c["exists"] = c["path"] in existing
        c["managed"] = c["path"] in meta_paths
    ...
    ctx = {
        ...
        "extras": extras,
        "schedule": schedule,
        "remaining": sum(1 for c in candidates if not c["exists"]),
        "notice": notice,
    }
    return request.app.state.templates.TemplateResponse(
...
async def scaffold_apply_async(
    ...
    if not chosen:
        # Nothing ticked (e.g. every category already exists) — bounce
        # back to the checklist with an explanation instead of a
        # silent redirect that reads as a broken submit.
        return RedirectResponse(
            f"/settings/entities/{slug}/scaffold?notice=nothing-selected",
            status_code=303,
        )
    writer = AccountsWriter(
        main_bean=settings.ledger_main,
