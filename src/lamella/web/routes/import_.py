...
def _entity_choices(conn) -> list[dict[str, str]]:
    """Active registry entities for the classify / preview pickers.
    Values are slugs (what `imports.entity` and downstream emit
    expect); labels prefer the display name."""
    from lamella.core.registry.service import list_entities
    return [
        {"slug": e.slug, "label": e.display_name or e.slug}
...
# NOTE: the .json route must register BEFORE /{import_id} — Starlette
# matches in registration order, and "/import/1.json" otherwise hits
# the detail route, fails the int parse, and 422s.
...
def classify_page(
    ...
    ctx = {
        ...
        "entities": _entity_choices(service.conn),
    }
    return request.app.state.templates.TemplateResponse(
...
@router.post("/{import_id}/delete")
def hard_delete_form(
    import_id: int,
    service: ImportService = Depends(get_import_service),
):
    """Form-post twin of the DELETE endpoint. The detail page's
    Hard-delete button submits here so it rides the standard form
    pipeline (CSRF hidden-input injection, confirm modal) instead of
    a hand-rolled fetch() that skipped both."""
    try:
