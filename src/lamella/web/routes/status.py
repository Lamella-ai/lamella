...
def _vector_index_card(
    ...
    if build:
        card.stats.append(("Last built", _fmt_ts(build["built_at"])))
...
def _paperless_card(
    ...
    card.stats = [
        ...
        (
            "Last full sync",
            _fmt_ts(sync_state["last_full_sync_at"]) if sync_state else "never",
        ),
        (
            "Last incremental sync",
            _fmt_ts(sync_state["last_incremental_sync_at"]) if sync_state else "never",
        ),
        (
            "Last sync status",
...
def _simplefin_card(
    ...
    if last_ingest:
        card.stats.append(
            ("Last ingest started", _fmt_ts(last_ingest["started_at"])),
        )
        card.stats.append(
