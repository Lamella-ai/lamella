...
def business_detail(
    ...
    cogs_accounts = sorted({
        e.account
        for e in entries
        if getattr(e, "account", "").startswith(f"Expenses:{slug}:COGS:")
    })
    has_cogs_accounts = bool(cogs_accounts)
    inventory = (
    ...
    ctx = {
        ...
        "inventory": inventory,
        "has_cogs_accounts": has_cogs_accounts,
...
def business_seed_cogs(
    ...
    """
    entity = conn.execute(
        "SELECT tax_schedule FROM entities WHERE slug = ?", (slug,),
    ).fetchone()
    if entity is None:
        raise HTTPException(status_code=404, detail="entity not found")
    if (entity["tax_schedule"] or "").upper() != "C":
        return RedirectResponse(
            f"/businesses/{slug}?error=cogs_schedule_c_only",
            status_code=303,
        )
