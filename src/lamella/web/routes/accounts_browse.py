...
def _render_account_detail(
    ...
    *,
    ...
    """Build + render the account detail page. The opening-balance
    POST's error path re-renders through here with ``error`` +
    ``form_values`` so the form keeps the user's input (ADR-0066)."""
    row = conn.execute(
        "SELECT * FROM accounts_meta WHERE account_path = ?",
        (account_path,),
    ).fetchone()
...
def account_detail_page(
    ...
    return _render_account_detail(
        account_path, request,
        conn=conn, reader=reader, saved=saved, opened=opened,
    )
...
def account_record_opening_balance(
    ...
    def _err(message: str):
        # ADR-0066: re-render the detail page with the submission —
        # the old ?error= redirect cleared the form AND the template
        # never displayed the code, so failures were fully silent.
        return _render_account_detail(
            account_path, request,
            conn=conn, reader=reader,
            error=message,
            form_values={
    ...
    try:
        ...
        return _err(
            f"Invalid amount {amount!r} — expected a number like 1234.56."
        )
    ...
    try:
        ...
        return _err(f"bean-check rejected the anchor: {exc}")
    except Exception as exc:  # noqa: BLE001
