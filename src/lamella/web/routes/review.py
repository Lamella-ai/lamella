...
def _build_receipt_link_index(
    ...
    """Resolve receipt_links for a batch of txn hashes, joined with
    paperless_doc_index so the review template can render a
    "Verify" button with the paperless_id + know whether the OCR
    looked implausible.

    Key: txn_hash. Value: {paperless_id, receipt_date, total_amount,
    ...
    rows = conn.execute(
        ...
        SELECT rl.txn_hash, rl.paperless_id, rl.txn_date,
               pdi.receipt_date, pdi.total_amount, pdi.vendor,
               pdi.mime_type
    ...
    for row in rows:
        ...
        try:
            if row["receipt_date"] and row["txn_date"]:
                d1 = _date.fromisoformat(str(row["receipt_date"])[:10])
