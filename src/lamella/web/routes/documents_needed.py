...
"""Documents-needed queue: the txn-first view of expenses awaiting finalization.

Per row: a transaction with at least one Expenses posting, not yet linked to
a document and not dismissed as "no document expected". Candidates come from
the local paperless_doc_index via the cascading matcher. Actions:

  * Link: attach a Paperless doc to the txn (DocumentLinker). Optional
    target_account, applied as a FIXME override if the txn has a FIXME
    posting.
  * Dismiss: record in document_dismissals so the row stops re-appearing.
"""
...
def _partial_url_for(
    *,
    ...
    return "/documents/needed/partial?" + urlencode(bits)
...
def _count_needs_receipt(
    ...
    *,
    ...
    through to ``/documents/needed?required_only=true``.
...
@router.get("/documents/needed", response_class=HTMLResponse)
def needed_page(
    request: Request,
    lookback_days: int = 90,
    ...
    loaded via hx-get against /documents/needed/partial so the user
    ...
    return request.app.state.templates.TemplateResponse(
        request, "documents_needed.html", ctx
    )


@router.get("/documents/receipts/needed", response_class=HTMLResponse)
def receipts_needed_page(
    request: Request,
    lookback_days: int = 90,
    include_linked: bool = False,
    include_dismissed: bool = False,
    ...
    """Receipt-typed sibling of /documents/needed.

    Old bookmarks at /receipts/needed 308-redirect here.

    TODO(ADR-0061 follow-up): the underlying ``needs_queue`` does not
    yet have a ``document_type`` discriminator — every "txn awaiting
    finalization" is treated as needing some kind of document, and
    receipts are by far the dominant document type. For the moment
    this endpoint returns the same data as ``/documents/needed`` so
    legacy ``/receipts/needed`` bookmarks land somewhere coherent. Once
    the queue grows a per-document-type filter (e.g. an invoice-needed
    queue), this view will narrow to ``document_type='receipt'``.
    """
    return needed_page(
        request,
        lookback_days=lookback_days,
...
@router.get("/documents/needed/partial", response_class=HTMLResponse)
def needed_partial(
    request: Request,
    lookback_days: int = 90,
    include_linked: bool = False,
    ...
    return request.app.state.templates.TemplateResponse(
        request, "partials/documents_needed_body.html", ctx
    )
...
@router.post("/documents/needed/{txn_hash}/link")
async def link_txn_to_doc(
    ...
    # Link writes document_links row + stamp in connector_links.bean; if bean-check
...
@router.post("/documents/needed/{txn_hash}/dismiss")
def dismiss_txn(
    txn_hash: str,
    request: Request,
    ...
    conn.execute(
        "INSERT INTO document_dismissals (txn_hash, reason, dismissed_by) "
        "VALUES (?, ?, 'user') "
...
@router.post("/documents/needed/bulk/dismiss")
async def bulk_dismiss(
    ...
    # Vanilla form post — bounce back to /documents/needed so the user
    # sees the updated queue.
    from fastapi.responses import RedirectResponse
    return RedirectResponse("/documents/needed", status_code=303)


@router.get(
    "/documents/needed/{txn_hash}/search", response_class=HTMLResponse
)
async def search_paperless_for_row(
    ...
    return request.app.state.templates.TemplateResponse(
        request, "partials/needs_document_search_results.html", ctx
    )


@router.post("/documents/needed/{txn_hash}/undismiss")
def undismiss_txn(
    txn_hash: str,
    request: Request,
    conn = Depends(get_db),
    ...
    conn.execute("DELETE FROM document_dismissals WHERE txn_hash = ?", (txn_hash,))
    if "hx-request" in {k.lower() for k in request.headers.keys()}:
        return HTMLResponse("")
    return Response(status_code=204)
