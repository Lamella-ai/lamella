...
"""Document-attach actions for staged + ledger /txn/{token} pages.

For ledger txns the /txn/{token} surface already lists linked documents
and offers a "link another" flow via :mod:`lamella.web.routes.documents`
keyed off the Beancount content hash. Staged rows have no content hash
yet, so the existing "Link" button on /documents/needed isn't reachable
...
:class:`lamella.features.receipts.linker.DocumentLinker` works without
modification: we simply pass the UUIDv7 as ``txn_hash``. The directive
shape is identical pre- and post-promotion, which keeps the document
joinable from either side via the same identity.

Endpoints exposed here:

* ``GET  /txn/{token}/document-section`` — partial that lists currently
  linked Paperless docs for this transaction plus a search box.
* ``GET  /txn/{token}/document-search?q=<query>`` — Paperless full-text
  search; returns a partial of candidate documents with Link buttons.
* ``POST /txn/{token}/document-link`` — append a ``custom "document-link"``
  block via :class:`DocumentLinker`. Returns 204 + ``HX-Refresh`` so the
  parent page reloads with the now-linked doc visible.
* ``POST /txn/{token}/document-unlink`` — strip the matching
  ``custom "document-link"`` block via :func:`remove_document_link`.
...
from lamella.features.receipts.linker import DocumentLinker, remove_document_link
...
def _require_uuidv7(token: str) -> None:
    if not _is_uuidv7_token(token):
        raise HTTPException(
            status_code=404,
            detail=(
                "/txn/{token}/document-* paths accept only a "
...
def _txn_date_for_link(
    *, staged: StagedRow | None, ledger: Transaction | None,
    ...
    """Pick a posting date for the document-link directive.
...
def _txn_amount_for_link(
    *, staged: StagedRow | None, ledger: Transaction | None,
    ...
    """Pick a representative amount for the document-link directive's
    ``lamella-txn-amount`` meta. For a ledger txn we re-use the documents
...
def _linked_docs(
    *, conn: sqlite3.Connection, txn_id: str, paperless_base: str,
    ...
    """Return the document-links cache rows for this txn id, joined to
...
@router.get(
    "/txn/{token}/document-section", response_class=HTMLResponse,
)
def document_section(
    ...
    """HTMX partial — renders the document-attach UI for a staged or
    ledger txn. Listed-docs come from the local document_links cache so
    ...
    return request.app.state.templates.TemplateResponse(
        request,
        "partials/_txn_document_section.html",
        {
            "token": token,
            "linked": linked,
            "is_staged": is_staged,
...
@router.get(
    "/txn/{token}/document-search", response_class=HTMLResponse,
)
async def document_search(
    ...
    back to ``/txn/{token}/document-link``."""
    ...
    return request.app.state.templates.TemplateResponse(
        request,
        "partials/_txn_document_search_results.html",
        {
            "token": token,
...
@router.post("/txn/{token}/document-link")
async def document_link(
    ...
    """Append a ``custom "document-link" "<token>"`` directive to
    ``connector_links.bean`` via the existing
    :class:`DocumentLinker`. The linker keys off
    ...
    try:
        ...
        log.error(
            "staged-document-link rejected by bean-check: doc=%d txn=%s "
            "err=%s",
    ...
    conn.execute(
        "INSERT INTO document_link_blocks "
...
async def paperless_unlink_mismatched(
    ...
    after the document-verify AI flagged the linked Paperless document
    ...
    1. **Local (authoritative).** Look up every ``document_links`` row
       for this paperless_id. For each, strip the corresponding
       ``custom "document-link"`` block from connector_links.bean and
       delete the SQLite row via :func:`remove_document_link`. Bean-
    ...
    cannot mass-delete document links because nothing in this code
    path takes a deletion action without a deliberate POST.
    """
    rows = conn.execute(
        "SELECT txn_hash FROM document_links WHERE paperless_id = ?",
        (paperless_id,),
    ).fetchall()
    if not rows:
        raise HTTPException(
            status_code=404,
            detail=f"no document-link rows for paperless #{paperless_id}",
        )
    removed_count = 0
    failed: list[str] = []
    for row in rows:
...
@router.post("/txn/{token}/document-unlink")
def document_unlink(
    token: str,
    request: Request,
    paperless_doc_id: int = Form(...),
    ...
    """Strip a previously written ``custom "document-link"`` block. The
    ...
    try:
        ...
        log.error(
            "staged-document-unlink rejected by bean-check: doc=%d "
            "txn=%s err=%s",
            paperless_doc_id, token[:12], exc,
        )
        raise HTTPException(
    ...
    if not removed:
        ...
        raise HTTPException(
            status_code=404,
            detail=(
                f"no document-link directive found for doc "
