...
class TxnRow:
    ...
    # the txn is in `receipt_links`; empty tuple otherwise. Lets the
...
def transactions_page(
    ...
    try:
        for r in conn.execute(
            "SELECT txn_hash, paperless_id FROM receipt_links "
            "ORDER BY txn_hash, paperless_id"
        ).fetchall():
            receipt_link_map.setdefault(r["txn_hash"], []).append(
                int(r["paperless_id"])
            )
    except Exception:  # noqa: BLE001 — receipt_links is optional
        pass
