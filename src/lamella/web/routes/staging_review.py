...
def _staged_list_context(
    ...
    *,
    ...
    # ADR-0063 §5: sub-threshold "Documents pending link" surface.
    # Per spec the section appears on /staging-review (which is now
    # /inbox post-ADR-0048; the same context object feeds both). The
    # heavy computation is gated behind an entries presence check so
    # an empty inbox skips the ledger walk entirely.
    docs_pending_link: list[dict] = []
    try:
        if entries:
            docs_pending_link = _build_docs_pending_link(conn, entries)
    except Exception:  # noqa: BLE001
        log.exception("/review docs_pending_link failed")
        docs_pending_link = []
    ...
    return {
        ...
        "docs_pending_link": docs_pending_link,
        "message": None,
    }


def _build_docs_pending_link(
    conn: sqlite3.Connection,
    entries: list,
    *,
    max_docs: int = 25,
) -> list[dict]:
    """ADR-0063 §5: collect sub-threshold (review-floor <= score < auto-link)
    document candidates for human adjudication on /inbox.

    Each returned dict carries doc metadata + the top scored ledger
    candidate so the template can render a "Documents pending link"
    section with Confirm / Reject row actions.
    """
    ...
    rows = conn.execute(
        ...
        "       document_type_name, content_excerpt "
        ...
        "ORDER BY paperless_id DESC LIMIT 200"
    ).fetchall()
    for r in rows:
        if len(out) >= max_docs:
            break
        doctype = (r["document_type"] or "").strip().lower() or None
        if doctype in {"statement", "tax"}:
            continue
        try:
            total = Decimal(str(r["total_amount"])) if r["total_amount"] else None
        except Exception:  # noqa: BLE001
            continue
        if total is None:
            continue
        try:
            doc_date = (
                _date.fromisoformat(str(r["document_date"])[:10])
                if r["document_date"] else
                _date.fromisoformat(str(r["created_date"])[:10])
            )
        except Exception:  # noqa: BLE001
            continue
        candidates = find_ledger_candidates(
            ...
            doc_vendor=r["vendor"],
            doc_doctype=doctype,
            doc_id=int(r["paperless_id"]),
            ledger_entries=entries,
            doc_correspondent=r["correspondent_name"],
            doc_content_excerpt=r["content_excerpt"],
            doc_last_four=r["payment_last_four"],
        ...
        if top.score >= AUTO_LINK_THRESHOLD:
            # Auto-link sweep should pick this up; don't surface here.
            continue
        out.append({
            "paperless_id": int(r["paperless_id"]),
            "title": r["title"] or f"Document #{r['paperless_id']}",
            ...
            "alt_count": max(0, len(candidates) - 1),
        })
    return out
...
# ─── ADR-0063: Documents pending link Confirm/Reject ─────────────────


@router.post("/review/staged/document-link/confirm", response_class=HTMLResponse)
def staged_review_document_link_confirm(
    request: Request,
    ...
    """Confirm a sub-threshold candidate from the "Documents pending
    link" surface — writes the link directive + DB row via
    :class:`DocumentLinker`, mirroring the auto-link write path so
    the document carries the same metadata regardless of which
    direction discovered the link (ADR-0063 §7).
    """
    ...
    # Pick the dominant Expenses/Liabilities/etc. posting amount.
    target_roots = ("Expenses", "Income", "Liabilities", "Equity")
    best: tuple[Decimal, str] | None = None
    for p in target.postings or ():
        acct = p.account or ""
        root = acct.split(":", 1)[0] if acct else ""
    ...
    if best is None:
        raise HTTPException(
            status_code=400, detail="transaction has no expense-side posting",
        )
    ...
    linker.link(
        ...
        txn_amount=best[0],
        match_method="user_review_confirm",
        match_confidence=1.0,  # explicit user confirmation = certainty
    ...
    return _redirect_to_list(
        request, message=f"linked_doc_{paperless_id}",
    )


@router.post("/review/staged/document-link/reject", response_class=HTMLResponse)
def staged_review_document_link_reject(
    request: Request,
    paperless_id: int = Form(...),
    txn_hash: str = Form(...),
    conn: sqlite3.Connection = Depends(get_db),
    ...
    """Reject a sub-threshold candidate. Writes a
    ``document-link-blocked`` directive (Phase-1 vocabulary) so the
    pair is not re-scored on the next sweep.
    """
    ...
    append_link_block(
        ...
        txn_hash=txn_hash,
        paperless_id=int(paperless_id),
        reason="user-rejected from /inbox sub-threshold review",
        run_check=False,
    )
    conn.execute(
        "INSERT OR REPLACE INTO document_link_blocks "
        "(paperless_id, txn_hash, blocked_at, reason) "
        "VALUES (?, ?, CURRENT_TIMESTAMP, ?)",
        (int(paperless_id), txn_hash, "user_review_reject"),
    )
    conn.commit()
    return _redirect_to_list(
        request, message=f"rejected_doc_{paperless_id}",
    )
...
def _resolve_account_path(
    ...
    * ``simplefin`` — looked up via ``accounts_meta.simplefin_account_id``.
    * ``reboot`` — derived from the staged row's ``raw`` envelope
      (``representative_account`` / ``postings`` captured by the reboot
      ingester). The first leg whose root is ``Assets:`` /
      ``Liabilities:`` wins; ``representative_account`` is the
      cheapest path when it already matches.
    * Any source whose ``source_ref`` carries an explicit
      ``account_path`` field.

    Returns ``None`` when no rule matches; callers decide whether
    that's a hard failure (legacy strict path) or a soft fallback
    (the AI-classify-job path falls through to source-less prompts).
    """
    if not isinstance(source_ref, dict):
        # Reboot rows can still resolve via ``raw`` even when
        # ``source_ref`` is malformed; everything else is a no-op.
        if source != "reboot":
            return None
    if source == "simplefin":
        if not isinstance(source_ref, dict):
        ...
        return None
    if source == "reboot":
        # Forward-fix path: a future reboot ingester writes
        # ``account_path`` directly into ``source_ref``. Honor it
        # first so newly-staged rows resolve via the generic
        # branch below as well.
        if isinstance(source_ref, dict) and isinstance(
            source_ref.get("account_path"), str,
        ):
            candidate = source_ref["account_path"]
            if candidate.startswith(("Assets:", "Liabilities:")):
                return candidate
        if not isinstance(raw, dict):
            return None
        # Prefer ``representative_account`` when it points at a real
        # bank-side leg (Assets:* / Liabilities:*). It's just the
        # first non-null posting captured at stage time, so it
        # isn't guaranteed to be the asset/liability leg.
        rep = raw.get("representative_account")
        if isinstance(rep, str) and rep.startswith(
            ("Assets:", "Liabilities:"),
        ):
            return rep
        # Fall back to scanning every captured posting for the
        # first Assets:/Liabilities: leg — same logic as
        # ``_classify_reboot_row`` runs against the live ledger
        # entry, but driven off the staged-row payload so the
        # caller doesn't have to re-load the ledger.
        for posting in raw.get("postings") or ():
            acct = (
                posting.get("account") if isinstance(posting, dict) else None
            )
            if isinstance(acct, str) and acct.startswith(
                ("Assets:", "Liabilities:"),
            ):
                return acct
        return None
...
def staged_review_classify(
    ...
    if row.source == "reboot":
        ...
        #
        # Reboot path runs BEFORE _resolve_account_path because
        # _classify_reboot_row re-derives the card account internally
        # by walking the matched ledger entry's postings — it does
        # not need a source_account hint. The historical
        # _resolve_account_path guard returned None for reboot rows
        # (their source_ref carries {file, lineno}, neither of which
        # the resolver knew how to map), causing a hard 400 before
        # this branch could run.
    ...
    # Resolve the source account path for non-reboot writes. Reboot
    # is handled above; only the SimpleFIN path below needs an
    # explicit source account because it appends a fresh balanced
    # transaction (vs. rerouting an existing FIXME leg).
    source_account = _resolve_account_path(
        conn, row.source, row.source_ref, raw=row.raw,
    )
...
def _build_pending_entry(
    ...
    source_account = _resolve_account_path(
        conn, row.source, row.source_ref, raw=getattr(row, "raw", None),
    )
    if not source_account:
        raise HTTPException(
            status_code=400,
...
def staged_review_classify_group(
    ...
    card_account = _resolve_account_path(
        ...
        raw=getattr(prototype, "raw", None),
    )
    if pattern_value:
        try:
...
def staged_review_ask_ai_modal(
    ...
    def _worker(ctx):
        ...
        # the synchronous handler uses. Threading ``raw`` lets the
        # reboot branch resolve via the captured ledger postings;
        # a None result is acceptable — the classifier widens to
        # cross-entity scope so the AI can still propose targets.
        source_account = _resolve_account_path(
        ...
        if not source_account:
            log.warning(
                "ask_ai (staged): _resolve_account_path returned None "
                "for row id=%s source=%s; falling through with "
                "source_account=None (cross-entity widening).",
                staged_id, row.source,
            )
            ctx.emit(
                "No source-account hint — running classifier with "
                "cross-entity scope.",
                outcome="info",
            )
        ...
        import asyncio
        # Track whether the AI was actually invoked so that a
        # None-proposal outcome can be classified correctly:
        #   ai_invoked=True  → AI ran but had no confident proposal
        #                       → outcome="info" (not a failure, just
        #                         insufficient confidence or no target)
        #   ai_invoked=False → pre-flight prevented the AI call entirely
        #                       → outcome="failure" (kept for that path)
        # Since _maybe_ai_classify swallows AIError internally (returns
        # None on transport failures), reaching this branch with
        # ai_invoked=True always means the AI ran but declined.
        ai_invoked = False
        ...
        if inline is None or inline.proposal is None:
            # AI ran and declined (low confidence / no valid target) →
            # surface as info, not failure. This matches the UX of the
            # /api/txn/{ref}/ask-ai endpoint which returns
            # "No confident proposal" without a failure badge.
            outcome_for_none = "info" if ai_invoked else "failure"
            ctx.emit(
                "No confident proposal.",
                outcome=outcome_for_none,
            )
            ctx.advance(1)
            return {
                "terminal_html": _render_terminal(None, blocked_flag=False),
