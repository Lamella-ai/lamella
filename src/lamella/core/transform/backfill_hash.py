...
existing document-link entry in ``connector_links.bean``, and populate the
``paperless_hash`` column on ``document_links`` in SQLite.
...
Implementation:
  1. Enumerate document_links rows where paperless_hash IS NULL.
  ...
  3. Dry-run: print a per-row report. --apply: append one amendment
     block per resolved row to connector_links.bean:
        ...
        2026-04-21 custom "document-link-hash-backfill" "<txn_hash>"
  ...
  4. Update the document_links.paperless_hash column.
...
async def plan(
    ...
    rows: list[sqlite3.Row] = list(
        conn.execute(
            ...
            "FROM document_links ORDER BY id ASC"
        )
    )
...
def _amendment_block(
    ...
    # ADR-0061: emit the v4 ``document-link-hash-backfill`` directive
    # type. The legacy ``receipt-link-hash-backfill`` shape is still
    # accepted by readers but no longer written.
    from lamella.features.receipts.directive_types import (
        DIRECTIVE_LINK_HASH_BACKFILL_NEW,
    )
    lines = [
        f'\n{today} custom "{DIRECTIVE_LINK_HASH_BACKFILL_NEW}" "{row.txn_hash}"',
    ...
    *,
    ...
    try:
        ...
        try:
            for row in resolvable:
                conn.execute(
                    "UPDATE document_links SET paperless_hash = ? WHERE id = ?",
        ...
        try:
            for row in resolvable:
                conn.execute(
                    "UPDATE document_links SET paperless_hash = NULL "
...
async def _amain(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Backfill lamella-paperless-hash on existing document_links."
    )
